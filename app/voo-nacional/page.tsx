...
import { dataAtual, calcularAtraso, calcularAtrasoMinutos, sugerirValorMorais, estimarChegadaPrevista } from "@/lib/calculos";
...
const DADOS_INICIAIS: DadosFormulario = {
  template: "voo-nacional-1-autor",
  num_autores: 1,
  autores: [{ nome: "", qualificacao: "", cpf: "", rg: "", email: "", endereco: "", data_nascimento: "", idoso: false }],
  companhia: "",
  tipo_rota: "direto",
  voos: [
...
const RASCUNHO_KEY = "rascunho_voo_nacional_1autor";
...
async function salvarHistorico(dados: DadosFormulario, tempoExtracaoMs?: number) {
  const novo: HistoricoItem = {
    id: uuidv4(),
    modulo: "Voo Nacional — 1 Autor",
    autor: dados.autores?.[0]?.nome ?? "Desconhecido",
...
export default function VooNacionalPage() {
  ...
  const [lote, setLote] = useState<DadosFormulario[]>([]);
  ...
  // Reseta a confirmação sempre que o valor dos danos morais mudar
  useEffect(() => {
    setMoraisConfirmado(false);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dados.valor_morais]);

  // Recalcula o valor por extenso dos danos morais (funcional — evita stale closure)
  useEffect(() => {
  ...
  // Recalcula tempo de atraso sempre que chegada prevista/real mudam
  useEffect(() => {
    const atraso = calcularAtraso(dados.chegada_prevista ?? "", dados.chegada_real ?? "");
    if (!atraso.texto) return;
    setDados((prev) => {
      if (prev.tempo_atraso === atraso.texto) return prev; // sem mudança — evita loop
      return { ...prev, tempo_atraso: atraso.texto, tempo_atraso_simples: atraso.simples };
    });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dados.chegada_prevista, dados.chegada_real]);

  // Recalcula dano moral automaticamente quando chegada prevista/real mudam (e ainda não foi confirmado)
  useEffect(() => {
    if (moraisConfirmado) return; // já confirmou: não sobrescreve
    const atrasoMin = calcularAtrasoMinutos(dados.chegada_prevista ?? "", dados.chegada_real ?? "");
    if (atrasoMin < 240) return; // < 4h: escritório não atua — não sugere
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
      ...
      dados.gestante_bebe, dados.condicao_especial,
      dados.autores?.[0]?.idoso, dados.recebeu_hospedagem]);
  ...
  useEffect(() => {
    ...
    function onKey(e: KeyboardEvent) {
      ...
      // Enter (sem modificadores): avança para a próxima aba.
      // Guard: não intercepta quando o foco está em textarea, select ou button
      // (nesses elementos Enter tem comportamento nativo importante).
      if (
        e.key === "Enter" &&
  ...
  useEffect(() => {
    ...
    fetch(`/api/rascunho?id=${encodeURIComponent(id)}`)
      ...
      .then((data) => {
        ...
        // Remove ?rascunho= da URL sem recarregar a página
  ...
  async function compartilharRascunho() {
    ...
    try {
      const res = await fetch("/api/rascunho", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ dados, camposIA, modulo: "voo-nacional" }),
      });
      if (!res.ok) throw new Error("Erro ao salvar rascunho");
      const { id } = await res.json() as { id: string };
      setLinkRascunho(`${window.location.origin}/voo-nacional?rascunho=${id}`);
    } catch {
      // silencioso — botão fica disponível para tentar novamente
    } finally {
  ...
  function avancarParaRevisao() {
    ...
    if (!autor?.qualificacao?.trim()) erros.push("Qualificação civil obrigatória");
    if (!dados.companhia) erros.push("Companhia aérea obrigatória");
    ...
    if (dados.valor_morais && parseFloat(dados.valor_morais) > 0 && !moraisConfirmado) erros.push("Confirme o valor dos danos morais clicando no botão amarelo");
  ...
  function handleExtraido(extraido: DadosExtraidos, campos: string[], novasConfiancas: ConfiancaExtracao = {}) {
    ...
    // ── Derivar chegada_prevista quando a IA não encontrou ──────────────────
    // Nível 1: AI extraiu direto → usa
    // Nível 2: usa data + horário de chegada do último voo original
    // Nível 3: estima somando duração de voo à partida (tabela ROTAS_BR)
    ...
    if (!chegPrev) {
      ...
      } else if (ultimoVoo?.data && ultimoVoo?.partida && ultimoVoo?.origem_sigla && ultimoVoo?.destino_sigla) {
        chegPrev = estimarChegadaPrevista(
          ultimoVoo.data,
          ultimoVoo.partida,
          ultimoVoo.origem_sigla,
          ultimoVoo.destino_sigla
        );
      }
    }

    // ── Derivar chegada_real quando a IA não encontrou ──────────────────────
    // Nível 1: AI extraiu direto → usa
    // Nível 2: usa data + horário de chegada do voo de realocação
    ...
    // vulnerável = idoso | gestante | bebê | condição especial (dados do autor ainda não carregados aqui)
    const valorSugerido = sugerirValorMorais(atrasoMin, perdaComp);

    const companhiaDetectada = extraido.companhia
      ? extraido.companhia.toUpperCase().includes("LATAM") ? "LATAM"
        : extraido.companhia.toUpperCase().includes("GOL") ? "GOL"
        : extraido.companhia.toUpperCase().includes("AZUL") ? "AZUL"
        : null
      : null;
    ...
    // Computa resumo para mostrar antes de navegar
    ...
    // NOTE: do NOT call setAba("formulario") here anymore
  }
  ...
  async function gerarLote() {
    ...
    try {
      ...
      if (!res.ok) {
        const j = await res.json() as { error?: string };
        throw new Error(j.error ?? "Erro ao gerar lote");
      }
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `Lote_Peticoes_${dataAtual().replace(/\//g, "-")}.zip`;
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex transition-colors duration-300">
      ...
      <aside className="w-64 bg-slate-900 fixed top-0 left-0 h-full flex flex-col z-20 shadow-xl">
        ...
        <div className="px-6 pt-7 pb-6 border-b border-slate-700">
          ...
          <h1 className="text-white font-bold text-base leading-tight">Voo Nacional</h1>
          <p className="text-slate-400 text-xs mt-0.5">1 Autor · Petição Inicial</p>
        </div>
            ...
            return (
              ...
              >
                ...
                <span className={`mt-0.5 flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                  ativo
                    ? "bg-white text-indigo-600 ring-2 ring-white/30"
                    : concluido
          ...
          <div className="mx-4 mb-4 p-3 bg-amber-500/10 border border-amber-500/30 rounded-lg">
            ...
            <div className="flex gap-1.5">
              <button
                ...
                className="flex-1 py-1 bg-amber-500 text-white text-xs font-medium rounded hover:bg-amber-400 transition-colors"
              >
                Continuar
              </button>
              <button
                onClick={descartarRascunho}
                className="flex-1 py-1 border border-amber-500/50 text-amber-400 text-xs font-medium rounded hover:bg-amber-500/10 transition-colors"
              >
                Descartar
              </button>
            </div>
        ...
        {/* Lote counter */}
        {lote.length > 0 && (
          <div className="mx-4 mb-4 px-3 py-2 bg-slate-800 rounded-lg flex items-center justify-between">
            <span className="text-slate-400 text-xs">Lote atual</span>
            <span className="bg-indigo-600 text-white text-xs font-bold px-2 py-0.5 rounded-full">
              {lote.length} {lote.length === 1 ? "caso" : "casos"}
            </span>
          </div>
        )}
        ...
        <main className="flex-1 px-8 py-7">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <div className="p-7">
              ...
              {aba === "qualificacao" && (
                <div>
                  <AbaQualificacao dados={dados} onChange={setDados} camposIA={camposIA} validarCampos={validarCampos} />
                ...
                <div>
                  ...
                  <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-700 flex flex-wrap gap-3 items-center justify-between">
                    ...
                    <div className="flex flex-wrap gap-3 items-center">
                      ...
                      <button
                        onClick={() => {
                          setLote((prev) => [...prev, dados]);
                          alert(`Caso adicionado ao lote. Total: ${lote.length + 1} caso(s).`);
                        }}
                        disabled={!todosMarcados}
                        className="px-5 py-2.5 border border-indigo-600 text-indigo-600 rounded-lg font-semibold hover:bg-indigo-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors text-sm"
                      >
                        + Adicionar ao Lote
                      </button>
                      ...
                      {lote.length >= 2 && (
                        <button
                          onClick={gerarLote}
                          disabled={gerando}
                          className="px-5 py-2.5 bg-slate-700 text-white rounded-lg font-semibold hover:bg-slate-800 disabled:opacity-40 transition-colors text-sm"
                        >
                          Gerar Lote ({lote.length}) .zip
                        </button>
                      )}
          ...
          >
            ...
            <div className="px-6 py-5 space-y-5 text-sm">
              ...
              <div className="border-b border-slate-100 dark:border-slate-800 pb-4">
                <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Autor</p>
                <p className="font-semibold text-slate-800 dark:text-slate-100">{dados.autores?.[0]?.nome || "—"}</p>
                {dados.autores?.[0]?.qualificacao && (
                  <p className="text-slate-500 dark:text-slate-400 text-xs mt-0.5 line-clamp-2">{dados.autores[0].qualificacao}</p>
                )}
              </div>
              ...
              {dados.observacoes_internas && (
                <div className="pb-2">
                  <p className="text-xs font-bold text-amber-600 dark:text-amber-400 uppercase tracking-wider mb-2">📝 Notas internas (não vai para a petição)</p>
                  <p className="text-amber-700 dark:text-amber-300 text-xs whitespace-pre-wrap">{dados.observacoes_internas}</p>
                </div>
              )}
