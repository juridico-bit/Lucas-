...
const DADOS_INICIAIS: DadosFormulario = {
  template: "voo-internacional-multi-autor",
  num_autores: 2,
  autores: [{ ...AUTOR_VAZIO }, { ...AUTOR_VAZIO }],
  companhia: "",
  tipo_rota: "direto",
  voos: [
...
const RASCUNHO_KEY = "rascunho_voo_internacional";

type Aba = "qualificacao" | "documentos" | "formulario" | "revisao";

const STEPS: { id: Aba; label: string; desc: string; num: number }[] = [
  { id: "qualificacao", label: "Qualificação", desc: "Autores e companhia aérea", num: 1 },
...
export default function VooInternacionalPage() {
  ...
  // Sugestão automática de danos morais (considera o autor mais vulnerável do grupo)
  useEffect(() => {
    ...
    // vulnerável = qualquer autor idoso OU gestante/bebê/condição especial
  ...
  async function compartilharRascunho() {
    ...
    try {
      const res = await fetch("/api/rascunho", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ dados, camposIA, modulo: "voo-internacional" }),
      });
      if (!res.ok) throw new Error("Erro ao salvar rascunho");
      const { id } = await res.json() as { id: string };
      setLinkRascunho(`${window.location.origin}/voo-internacional?rascunho=${id}`);
    } catch {
  ...
  /** Converte nome extraído pela IA ("ITA Airways", "Air France"…) para a chave do sistema */
  function normalizarCompanhia(nome: string | undefined): CompanhiaInternacionalKey | "" {
    ...
    // 2. Por nome fantasia (case-insensitive)
  ...
  function handleExtraido(extraido: DadosExtraidos, campos: string[], novasConfiancas: ConfiancaExtracao = {}) {
    ...
    // Para voos internacionais não temos tabela de duração, usamos dados extraídos diretamente
  ...
  async function gerarPeca() {
    ...
    try {
      const res = await fetch("/api/gerar-peca-internacional", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      <aside className="w-64 bg-slate-900 fixed top-0 left-0 h-full flex flex-col z-20 shadow-xl">
        ...
        <div className="px-6 pt-7 pb-6 border-b border-slate-700">
          ...
          <p className="text-slate-400 text-xs mt-0.5">
            {dados.num_autores ?? 2} {(dados.num_autores ?? 2) === 1 ? "Autor" : "Autores"} · Petição Inicial
          </p>
        </div>
        ...
        <main className="flex-1 px-8 py-7">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
            <div className="p-7">
              ...
              {aba === "qualificacao" && (
                <div>
                  <AbaQualificacaoInternacional dados={dados} onChange={setDados} camposIA={camposIA} validarCampos={validarCampos} />
