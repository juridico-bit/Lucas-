...
export async function POST(request: NextRequest) {
  try {
    ...
    // Template fixo para 1 autor — sem fallback para versão "novo"
    const templatePath = join(process.cwd(), "templates", "voo-internacional-1-autor.docx");
    if (!existsSync(templatePath)) {
      return NextResponse.json(
        { error: "Template 'voo-internacional-1-autor.docx' não encontrado. Faça o upload pelo painel Admin." },
        { status: 404 }
      );
    }
    ...
    const [descCompromissoReescrito, descDetalheReescrito] =
      await Promise.all([
        enqueueAI(() => reescreverCompromisso(dados.desc_compromisso ?? ""), "reescrever-compromisso-int-1a"),
        enqueueAI(() => reescreverTerceiraPessoa(dados.desc_compromisso_detalhe ?? ""), "reescrever-detalhe-int-1a"),
      ]);

    const autores = dados.autores ?? [];
    // Sempre 1 autor neste módulo
    const numAutores = 1;
    ...
    const valorMoraisTotal = valorMoraisPorAutor; // 1 autor — sem multiplicação
    ...
    const { foro: foroDesc, comarca: comarcaNome } = foroAutor
      ...
      : autorEmSP
        ...
        : parseForo(companhiaInfo?.comarca ?? "");
    ...
    // ── NOMES_AUTORES = apenas o nome do autor 1 ─────────────────────────────
    const NOMES_AUTORES = nome1;
    ...
    const nomeAutor1 = nome1.replace(/[/\\:*?"<>|]/g, "") || "Autor";
    const nomeArquivo = `Inicial Internacional - ${nomeAutor1}.docx`;
